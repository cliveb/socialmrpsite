+(function () {
    ...
    for (var h = 0, i = 0; i < screen.height; h++, i += e + 2) {
        ...
        for (var j = 0, k = 0; k < screen.width; j++, k += f + 2) {
            ...
            if (h != 1) {
                var q = a(p[0], p[1], p[2]), r = b(p, [0, 0, 0], .2), s = a(r[0], r[1], r[2]), t = $("<img />").attr("data-src", "holder.js/" + f + "x" + e + "?bg=" + q + "&fg=" + q + "font=Material+Icons&size=30&auto=true&text=&#mood").css({ top: i + "px", left: k + "px" });
                $("#background").append(t);
            }
        }
    }
